...
def polygon_transform(geom_polygon, coord_original, coord_target):
    ...
    for m in range(geom_poly_o_count):
    ...
    if boundary_polygon_count == 0:
        ...
        for i in range(boundary_polygon_count):
            ...
            ring = ogr.Geometry(ogr.wkbLinearRing)
            for j in range(boundary_polygon_ring_point_count):
...
def multi_polygon_transform(geom_polygon, coord_original, coord_target):
    ...
    for m in range(geom_polygon_count):
...
def polyline_transform(geom_polyline, coord_original, coord_target):
    ...
    for i in range(geom_polyline_count):
...
def multi_polyline_transform(geom_polyline, coord_original, coord_target):
    ...
    for m in range(geom_polyline_count):
...
def multi_point_transform(geom_point, coord_original, coord_target):
    ...
    for m in range(geom_point_count):
...
def transform_vector(psz_file, dst_file, coord_original, coord_target):
    gdal.SetConfigOption('GDAL_FILENAME_IS_UTF8', 'YES')
    # fcode = open(write_file_name + '.cpg', 'r')
    # fcodelines = fcode.readlines()
    # code_name = fcodelines[0].strip('\n')
    # fcode.close()
    gdal.SetConfigOption('SHAPE_ENCODING', "UTF-8")  # code_name
    ogr.RegisterAll()

    try:
        oDS = ogr.Open(psz_file, 0)
    ...
    oDDS = oDriver.CopyDataSource(oDS, dst_file, None)
    # oDDS = ogr.Open(dst_file, 1)
    if oDDS is None:
        print(f'{dst_file} : Can not create such new file or directory')
    # 获取数据源中的图层个数，shp数据图层只有一个，gdb、dxf会有多个
    # iLayerCount = oDDS.GetLayerCount()
    # print(f'图层个数 = {iLayerCount}')
    oLayer = oDDS.GetLayerByIndex(0)
    if oLayer is None:
        print(f'获取图层失败 : {dst_file}')
    # 对图层进行初始化
    oLayer.ResetReading()
    # 输出图层中的要素个数
    num = oLayer.GetFeatureCount(0)
...
if __name__ == "__main__":
    src_data = "chla_2207.shp"
    dst_data = ".chla_2207_gcj02.shp"
    transform_vector(src_data, dst_data, "wgs84", "gcj02", associate_file=False)
